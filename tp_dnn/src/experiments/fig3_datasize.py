"""
fig3_datasize.py – Figure 3: Classification error vs number of training samples.

Sweeps over different training-set sizes (config.FIG3_TRAIN_SIZES) and compares
...
from src.config import (
    FIG3_TRAIN_SIZES, RBM_EPOCHS, DNN_EPOCHS, LR, BATCH_SIZE, SEED,
...
def run(seed=None):
    """Run the data-size sweep experiment and produce Figure 3."""
    seed = seed if seed is not None else SEED
    device = get_device()
    print(f"Device: {device}")

    run_dir = create_run_directory("fig3_datasize")
    ...
    for n_train in FIG3_TRAIN_SIZES:
        print(f"\n{'='*50}")
        ...
        train_loader, test_loader = load_mnist_binarized(
            ...
            train_subset=n_train, seed=seed,
        )

        # Collect data for DBN pretraining
        all_x = []
    ...
    # Plot
    plot_error_curve(
        FIG3_TRAIN_SIZES, pre_errors, rand_errors,
        title="Fig 3 – Error vs training set size",
        xlabel="Number of training samples",
        out_path=str(FIG_DIR / "fig3_datasize.png"),
    )

    # Save metrics
    metrics = {
        "experiment": "fig3_datasize",
        "seed": seed,
        "architecture": {
            "layer_sizes": layer_sizes, "num_classes": NUM_CLASSES,
        },
        "hyperparameters": {
            "rbm_epochs": RBM_EPOCHS, "dnn_epochs": DNN_EPOCHS,
            "lr": LR, "batch_size": BATCH_SIZE,
        },
        "train_sizes": FIG3_TRAIN_SIZES,
    ...
    print(f"\nFigure saved to outputs/figures/fig3_datasize.png")
